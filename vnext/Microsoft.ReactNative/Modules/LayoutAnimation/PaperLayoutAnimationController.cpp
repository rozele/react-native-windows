...
void PaperLayoutAnimationController::ConfigureLayoutAnimation(
    winrt::Microsoft::ReactNative::JSValue &&config) noexcept {
  if (config.IsNull()) {
    Reset();
    return;
  }

  const auto globalDuration = config["duration"].AsInt32();
...
void PaperLayoutAnimationController::OnComplete(int64_t tag) noexcept {
  ...
  if (iter != m_tagsToOperations.end()) {
    for (auto i = 0; i < iter->second.size(); ++i) {
      ...
      // TODO: we skip layout operations for unfinished layout changes. This
      // may be a problem for components that expect `onLayout` events to
      // deterministically fire for each layout event.
...
void PaperLayoutAnimationController::SetHost(INativeUIManagerHost *host) noexcept {
  m_host = host;
}
...
void PaperLayoutAnimationController::AnimateLayoutProps(
  ...
  if (!IsNaNOrZero(element.Width()) && !IsNaNOrZero(element.Height())) {
    ...
    if (const auto storyboard = m_updateLayoutAnimation.CreateAnimation(node, left, top, width, height, std::move(onComplete))) {
      storyboard.Begin();
