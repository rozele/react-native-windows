...
xaml::Media::Animation::Storyboard PaperUpdateLayoutAnimation::CreateAnimation(
  ...
  // TODO: animate size layout
  const auto element = node.GetView().as<xaml::UIElement>();
  const auto stableLeft = winrt::Microsoft::ReactNative::ViewPanel::GetLeft(element);
  const auto stableTop = winrt::Microsoft::ReactNative::ViewPanel::GetTop(element);
